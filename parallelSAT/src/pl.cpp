...
int main(int argc, char *argv[]) {
	...
	std::ifstream fp;
		fp.open(paramteterFile,ios::in);
		if(!fp.is_open()){
			perror("read parameters fails");
					exit(EXIT_FAILURE);
		}
		vector<string> parameterVector;
		parameterVector.reserve(numberOfThreads);
		string buff;
		for(int i = 0; i < numberOfThreads; i++){
			std::getline(fp,buff);
			parameterVector.push_back(buff);
		}
	readFile(fileName);
	//debugProblem();
	...
	{
		Process process = Process(parameterVector[omp_get_thread_num()]);
	}
	cout << "sat?" <<satG << endl;
...
void parseOptions(int argc, char *argv[]){
	...
	struct option longopts[] ={{"output",no_argument,0,'o' },
							   ...
							   {"paramteterFile", required_argument,0, 'p'},
	...
	int result;
	int option_index = 0;
	while ((result = getopt_long(argc, argv, "ohs:p:t:",longopts,&option_index)) != -1){
		switch (result) {
...
Process::Process(string parameter){
	char* pr = strdup(parameter.c_str());
    ...
    while(token != 0){
    	...
    	case 'e':
    		...
    		break;
    	case 'f':
    		fct = atoi(strtok(NULL, " "));
    		break;
    	case 'i':
    		ict = atoi(strtok(NULL, " "));
    		break;
    	case 'p':
    		flips = atoi(strtok(NULL, " "));
    		break;
    	case 's':
...
int Process::getFlipCandidate(int cIndex){
	...
	for (std::vector<int>::const_iterator i = vList.begin(); i != vList.end(); ++i){
		...
		if(ct){
			if(bre == 0 && rand()%ULLONG_MAX > tabuS[*i]) return *i;
			bre += ct*tabuS[*i];
		}
		sum+= lookUpTable[bre];
		probs[j]= sum;
